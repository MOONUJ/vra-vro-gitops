...
def get_workflow_dirs(root_dir):
    ...
    vro_root = os.path.join(root_dir, "content", "orchestrator")
...
def get_target_workflow_path(root_dir, category_path, workflow_name):
    ...
    cap_path = os.path.join(root_dir, "content", "orchestrator", "Workflows", category_path, workflow_name)
    if os.path.exists(cap_path):
        return cap_path

    # Check if lowercase w path exists
    lower_path = os.path.join(root_dir, "content", "orchestrator", "workflows", category_path, workflow_name)
...
def pull_all(client, config, root_dir, target_ids=None):
    ...
    for act in discovered_actions:
        ...
        try:
            ...
            local_dir = os.path.join(root_dir, "content", "orchestrator", "actions", module, act_name)
            os.makedirs(local_dir, exist_ok=True)
    ...
    for cfg in discovered_configs:
        ...
        try:
            ...
            local_dir = os.path.join(root_dir, "content", "orchestrator", "configurations", category_path)
            os.makedirs(local_dir, exist_ok=True)
    ...
    for res in discovered_resources:
        ...
        try:
            ...
            local_dir = os.path.join(root_dir, "content", "orchestrator", "resources", category_path, res_name)
            os.makedirs(local_dir, exist_ok=True)
...
def get_local_configurations(root_dir):
    config_files = []
    configs_root = os.path.join(root_dir, "content", "orchestrator", "configurations")
...
def get_local_resources(root_dir):
    resource_dirs = []
    resources_root = os.path.join(root_dir, "content", "orchestrator", "resources")
...
def push_all(client, config, root_dir, dry_run=False, force_bootstrap=False, target_ids=None):
    ...
    for cfg_file in local_cfg_files:
        try:
            ...
            rel_path = os.path.relpath(os.path.dirname(cfg_file), os.path.join(root_dir, "content", "orchestrator", "configurations"))
    ...
    for res_dir in local_res_dirs:
        try:
            ...
            rel_path = os.path.relpath(os.path.dirname(res_dir), os.path.join(root_dir, "content", "orchestrator", "resources"))
...
def pull_all_vra(client, config, root_dir, target_names=None):
    ...
    auto_root = os.path.join(root_dir, "content", "automation")
